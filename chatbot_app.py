...
def process_input(input_content, input_type, criteria, custom_prompt)
    ...
    try
        ...
        messages = [
            ...
            {role user, content f이 {input_type}를 {criteria}에 맞춰 평가해줘 {input_content[1000]}...}  # 메시지 길이 제한
        ]
        ...
        return f처리 중 오류가 발생했습니다 {str(e)}
    except Exception as e
        return f알 수 없는 오류가 발생했습니다 {str(e)}
...
def main()
    ...
    # 사이드바에 설정 추가
    with st.sidebar
        st.markdown(h2 class='sidebar-title'⚙️ 설정h2, unsafe_allow_html=True)
        set_openai_api_key()

        # 평가 기준 입력
        st.markdown(h2 class='sidebar-title'📈 평가 기준h2, unsafe_allow_html=True)
    ...
    # 입력 및 피드백 생성 섹션
    with col1
        st.markdown(h2 class='section-title'📥 입력h2, unsafe_allow_html=True)
    ...
    # 피드백 섹션
    with col2
        st.markdown(h2 class='section-title'💬 피드백h2, unsafe_allow_html=True)
        feedback = st.session_state.get('feedback', 아직 생성된 피드백이 없습니다.)
        st.markdown(fdiv class='feedback-box'{feedback}div, unsafe_allow_html=True)
